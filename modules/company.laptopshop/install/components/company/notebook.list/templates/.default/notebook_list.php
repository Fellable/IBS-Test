...
<form method="GET" class="mb-4 p-2 border">
    <div class="d-flex flex-wrap align-items-start mb-2 w-100">
        ...
        <div class="d-flex flex-column flex-grow-1 text-center">
            ...
            <select name="page_size" id="page_size" class="form-control">
                ...
                <option value="10" <?= ($arResult['REQUEST']->getQuery("page_size") == 10) ? 'selected' : '' ?>>10
                </option>
                <option value="20" <?= ($arResult['REQUEST']->getQuery("page_size") == 20) ? 'selected' : '' ?>>20
                </option>
            </select>
        </div>
    </div>
    <div class="d-flex flex-column align-self-stretch w-100">
        <button type="submit"
                class="btn btn-primary h-100 w-100"><?= Loc::getMessage('LAPTOPSHOP_FORM_APPLY') ?></button>
    </div>
</form>
...
<div class="row p-2">
    ...
    if (!empty($arResult['DATA'])):
        foreach ($arResult['DATA'] as $notebook): ?>
